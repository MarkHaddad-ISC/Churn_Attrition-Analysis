...
from Prompts.TimelinePrompt import *
...
file_path = '/Users/haddadm/CursorXopenPYXL/Outputs/OUTPUT_TimelineData.xlsx'
sheet_name = 'RMSOnly'
...
batch_size = 10
starting_row = 300 #change to actual starting row value if it errors out, no offset neccesary, make this number the exact number of the row you want to start anaylsis on, row 1 is the header row no naturally we start at 2
...
# Extract entries from columns A, D, and L and concatenate them
entries = []
for row in range(starting_row, FinalRowPlus1):
    col_a_value = sheet[f'A{row}'].value or ""
    col_d_value = sheet[f'D{row}'].value or ""
    col_l_value = sheet[f'L{row}'].value or ""
    
    # Concatenate the three columns with separators
    concatenated_entry = f"Meeting Notes: {col_a_value} | Subject: {col_d_value} | Reason Lost Notes: {col_l_value}"
    entries.append(concatenated_entry)
...
for batch_start in range(0, total_entries, batch_size):
    ...
    for item in batch_entries:
        for attempt in range(MAX_RETRIES):
            try:
                singularLineAnalysis = clean_string(vanillaChatBot(timelinePrompt + item, systemPythonPrompt, 'gpt-4o'))
                productReasonSummary = extract_bracketed_value(singularLineAnalysis, 3)
    ...
    for local_row_num, values in analyzed_batch.items():
        ...
        for col_num, value in enumerate(values, start=14):  # Start at column N
